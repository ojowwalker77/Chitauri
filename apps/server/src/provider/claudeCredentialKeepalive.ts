...
//
// Opt in:   CHITAURI_CLAUDE_KEEPALIVE=1
// Tune:     CHITAURI_CLAUDE_KEEPALIVE_MINUTES=<n>   (default 30)
...
export function isClaudeCredentialKeepaliveEnabled(
  ...
  return platform === "darwin" && envFlagEnabled(env.CHITAURI_CLAUDE_KEEPALIVE);
}
...
export function resolveClaudeCredentialKeepaliveIntervalMs(env: NodeJS.ProcessEnv): number {
  const raw = env.CHITAURI_CLAUDE_KEEPALIVE_MINUTES?.trim();
