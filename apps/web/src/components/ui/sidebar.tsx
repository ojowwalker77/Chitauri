...
import { disclosurePeekClassName } from "~/lib/disclosureMotion";
import { useSidebarEdgePeek } from "~/hooks/useSidebarEdgePeek";
import { SIDEBAR_PEEK } from "~/lib/sidebarPeek";
 ...
 */
const SIDEBAR_OFFCANVAS_MOTION_CLASS = "duration-sheet ease-drawer motion-reduce:transition-none";
...
function Sidebar({
  ...
  const peek = useSidebarEdgePeek({
    enabled: !isMobile && collapsible === "offcanvas" && state === "collapsed",
  });
  ...
  return (
    <SidebarInstanceContext.Provider value={instanceContextValue}>
      <div
        ...
        data-peek={peek.peeking ? "true" : undefined}
        data-side={side}
        data-slot="sidebar"
        data-state={state}
        data-variant={variant}
      >
        {/* Invisible edge strip that opens the peek. Rendered only once armed, so
            the pointer that collapsed the sidebar cannot immediately reopen it. */}
        {peek.armed ? (
          <div
            aria-hidden="true"
            className={cn(
              "fixed inset-y-0 z-[90] hidden md:block",
              side === "left" ? "left-0" : "right-0",
            )}
            data-slot="sidebar-peek-trigger"
            style={{ width: `${SIDEBAR_PEEK.triggerWidthPx}px` }}
            {...peek.triggerProps}
          />
        ) : null}
        ...
        <div
          className={cn(
            ...
            // The peek slides the collapsed panel back over the content rather
            // than reopening the sidebar: the gap element stays 0-width, so the
            // main content never reflows underneath it. Raised above content only
            // while peeked, so the resting sidebar keeps its usual stacking.
            disclosurePeekClassName(
              peek.peeking,
              side === "left" ? "translate-x-(--sidebar-width)" : "-translate-x-(--sidebar-width)",
            ),
            // Above the app chrome it now floats over (the documented ladder puts
            // app surfaces at z-90), but below hover cards (100) and modals (200+),
            // including the sidebar's own row hover cards.
            peek.peeking && "z-[95]",
        ...
        >
          ...
          <div
            className={cn(
              "relative z-0 flex h-full w-full flex-col group-data-[variant=floating]:rounded-xl",
              !transparentSurface &&
                "bg-sidebar group-data-[variant=floating]:border group-data-[variant=floating]:border-sidebar-border",
              innerClassName,
            )}
            data-sidebar="sidebar"
...
function SidebarRail({
  ...
  const resizeStateRef = React.useRef<{
    /** Removes the window listeners this drag installed. */
  ...
  const handleWindowPointerEnd = React.useCallback(
    (event: PointerEvent) => {
      const resizeState = resizeStateRef.current;
      // `lostpointercapture` fires for the same pointer, so the id check keeps a
      // stray pointer from ending someone else's drag.
  ...
  // Unmounting mid-drag would otherwise leave the body stuck with `col-resize`
  // and `user-select: none` for the rest of the session.
  React.useEffect(
    () => () => {
      const resizeState = resizeStateRef.current;
  ...
  const handlePointerDown = React.useCallback(
    (event: React.PointerEvent<HTMLButtonElement>) => {
      ...
      // The drag runs on window listeners, not on the rail's own pointer events.
      // Pointer capture is not guaranteed to survive: a re-render can detach the
      // rail, and a release outside the window never reaches it at all. When that
      // happened the drag never ended — the ref stayed set, the body kept
      // `col-resize`, and the next pointer move resized the sidebar again.
      const { signal } = abort;
      window.addEventListener("pointermove", handleWindowPointerMove, { signal });
      for (const type of ["pointerup", "pointercancel", "lostpointercapture"] as const) {
        window.addEventListener(type, handleWindowPointerEnd, { signal });
      }
      // A window that loses focus mid-drag (app switch, window drag) may never
  ...
  return (
    <main
      className={cn(
        ...
        "md:peer-data-[variant=inset]:peer-data-[state=collapsed]:ms-2 md:peer-data-[variant=inset]:m-2 md:peer-data-[variant=inset]:ms-0",
        className,
      )}
...
function SidebarContent({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <ScrollArea hideScrollbars scrollFade className="h-auto min-h-0 flex-1">
      <div
        className={cn(
          "flex w-full min-w-0 flex-col gap-1.5 group-data-[collapsible=icon]:overflow-hidden",
          className,
        )}
  ...
  "peer/menu-button flex w-full cursor-pointer items-center gap-1.5 overflow-hidden rounded-lg p-1.5 text-left text-sm outline-hidden ring-ring/60 transition-[width,height,padding,background-color,color,scale] duration-press ease-out active:scale-[0.96] motion-reduce:active:scale-100 hover:bg-[var(--sidebar-accent)] focus-visible:ring-2 active:bg-[var(--sidebar-accent-active)] active:text-[var(--sidebar-accent-foreground)] disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pe-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-[var(--sidebar-accent-active)] data-[active=true]:font-medium data-[active=true]:text-[var(--sidebar-accent-foreground)] data-[state=open]:hover:bg-[var(--sidebar-accent)] group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! [&>span:last-child]:truncate [&>svg:not([class*='size-'])]:size-4 [&>svg]:shrink-0",
  {
    ...
    variants: {
      ...
      variant: {
        ...
        outline: "border border-sidebar-border bg-background hover:bg-[var(--sidebar-accent)]",
      },
    },
  },
);
...
function SidebarMenuSkeleton({
  ...
  return (
    <div
      className={cn("flex h-8 items-center gap-1.5 rounded-lg px-1.5", className)}
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<"ul">) {
  return (
    <ul
      className={cn(
        "mx-2.5 flex min-w-0 translate-x-px flex-col gap-0.5 border-sidebar-border border-l px-1.5 py-0.5",
        "group-data-[collapsible=icon]:hidden",
        className,
      )}
...
function SidebarMenuSubButton({
  ...
  const defaultProps = {
    className: cn(
      "-translate-x-px flex h-7 min-w-0 cursor-pointer items-center gap-1.5 overflow-hidden rounded-lg px-1.5 text-sidebar-foreground outline-hidden ring-ring/60 hover:bg-[var(--sidebar-accent)] focus-visible:ring-1 active:bg-[var(--sidebar-accent-active)] active:text-[var(--sidebar-accent-foreground)] disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 [&>span:last-child]:truncate [&>svg:not([class*='size-'])]:size-4 [&>svg]:shrink-0",
