...
const stageMacAppSnapHelper = Effect.fn("stageMacAppSnapHelper")(function* (
  ...
  yield* runCommand(
    ChildProcess.make({
      cwd: stageAppDir,
      ...commandOutputOptions(verbose),
    })`node ${buildScript} --mode release --arch ${arch} --output ${outputPath}`,
  );
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const stagedVendorDir = path.join(stageRoot, "vendor/effect");
  const distDirs = {
  ...
  yield* fs.copy(path.join(repoRoot, "vendor/effect"), stagedVendorDir);
  ...
  const stagePackageJson: StagePackageJson = {
    ...
    dependencies: {
      ...rebaseVendoredDependenciesForStaging(resolvedServerDependencies),
      ...rebaseVendoredDependenciesForStaging(resolvedDesktopRuntimeDependencies),
    },
    devDependencies: {
    ...
    overrides: {
      ...
      ...rebaseVendoredDependenciesForStaging(resolvedOverrides),
    },
  };
